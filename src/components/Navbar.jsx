...
const Navbar = () => {
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass-dark py-3 shadow-lg' : 'bg-transparent py-5'
                }`}
        >
            <div className="container mx-auto px-5 md:px-11 flex justify-between items-center">
                {/* Logo */}
                        ...
                        <div className="flex flex-col p-5 gap-3">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    to={link.path}
                                    className={`text-base font-medium py-1.5 border-b border-gray-800
